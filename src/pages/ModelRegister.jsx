...
export const ModelRegister = () => {
  ...
  const addRightLine = (plan) => {
    setModelForm((prev) => ({
      ...
      pricing: {
        ...
        [plan]: {
          ...prev.pricing[plan],
          rights: [...(prev.pricing[plan].rights || []), ''],
        },
      },
    }));
  };

  const updateRightLine = (plan, index, value) => {
    setModelForm((prev) => {
      const nextRights = [...(prev.pricing[plan].rights || [])];
      nextRights[index] = value;
      return {
        ...prev,
        pricing: {
          ...prev.pricing,
  ...
  const removeRightLine = (plan, index) => {
    setModelForm((prev) => {
      const currentRights = prev.pricing[plan].rights || [];
      const nextRights = currentRights.filter((_, idx) => idx !== index);
      return {
        ...prev,
        pricing: {
          ...prev.pricing,
            ...
            {activePlans.map((plan) => (
              <div key={plan} className="border border-gray-200 rounded-lg p-4 space-y-4">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {(modelForm.pricing[plan].rights || []).length === 0 && (
                      <p className="text-xs text-gray-500">+ 버튼을 눌러 권한 항목을 추가하세요.</p>
                    )}
                    {(modelForm.pricing[plan].rights || []).map((right, index) => (
                      <div key={`${plan}-right-${index}`} className="flex items-center gap-2">
                        <input
                          type="text"
                          value={right}
                          onChange={(e) => updateRightLine(plan, index, e.target.value)}
                          className="flex-1 rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                          placeholder="예: 상업적 사용"
                        />
                        <button
                          type="button"
                          onClick={() => removeRightLine(plan, index)}
                          className="inline-flex items-center justify-center rounded-lg border border-gray-200 bg-white p-2 text-gray-500 hover:text-red-500 hover:border-red-200"
                          aria-label="권한 삭제"
                        >
                          <X className="h-4 w-4" />
                        </button>
                    ...
                    <button
                      type="button"
                      onClick={() => addRightLine(plan)}
                      className="inline-flex items-center gap-2 rounded-lg border border-dashed border-gray-300 px-3 py-2 text-sm text-gray-600 hover:border-blue-300 hover:text-blue-600"
                    >
